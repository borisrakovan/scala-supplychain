...
// copied from https://stackoverflow.com/questions/7539831/scala-draw-table-to-console

object Tabulator {
  def format(table: Seq[Seq[Any]]): String = table match {
    case Seq() => ""
    case _ =>
      val maxRowSize = table.map(_.size).max
      val normalizedTable = for (row <- table) yield row.padTo(maxRowSize, "")
      val sizes =
        for (row <- normalizedTable)
          yield (for (cell <- row) yield if (cell == null) 0 else cell.toString.length)
      val colSizes = for (col <- sizes.transpose) yield col.max
      val rows = for (row <- normalizedTable) yield formatRow(row, colSizes)
